...
class Star(val id: Int, val coords: StarCoords, val magnitude: Double, val name: Option[String]) {
  ...
  /** Determines the location of this star within a given image of a star map.
    * This method effectively converts the coords of this star (a StarCoords) into a Pos within the given image.
    * For instance, if this star has coords of (0,0), and a 400-by-400 pixel image is given, this method returns a Pos of (200,200). */

  def posIn(skyPic: o1.Pic): world.Pos = coords.toImagePos(skyPic)

  /** Returns a string description of the star. The format of the string varies slightly depending on whether the star has a name or not.
    * For a named star, the description has the form "#ID NAME (COORDS)"; e.g., "#39801 Betelgeuse (x=0.02, y=0.99)".
    * In case of an unnamed star, the name and one of the spaces are omitted; e.g.,"#39810 (x=0.016, y=0.30)". */

  override def toString: String = s"#$id ${
    name match {
      case Some(starName) => starName + " "
      case None => ""
    }
  }(${coords.toString})"
}
