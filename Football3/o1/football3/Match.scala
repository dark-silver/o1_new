...
class Match(val home: Club, val away: Club) {
  ...
  def winner: Option[Club] = if(isHomeWin) Some(home) else if(isAwayWin) Some(away) else None
  ...
  def winningScorer: Option[Player] = {
    if(isHomeWin) {
      Some(homeScorers(homeScorers.length - goalDifference))
    } else if(isAwayWin) {
      Some(awayScorers(awayScorers.length + goalDifference))
    } else None
  }
  ...
  def chooseBetter(another: Match): Match = if (this.isBiggerDiffThan(another)) this else another
  private def isBiggerDiffThan(another: Match): Boolean = this.goalDifference.abs > another.goalDifference.abs
